...
通过自然语言澄清规则，复用纸牌 DSL 或让 Agent 编写新的游戏逻辑；验证后确认、模拟、与电脑试玩。旧版阶段玩法支持离线导出。
...
新增代码生成通路：内置 DSL 无法描述时，Agent 制定行为测试、生成 JavaScript、执行验证并依据错误修复。程序通过 QuickJS 在服务端子进程中运行，接入同一牌桌。详见 [生成协议及边界](docs/generated-game-protocol.md)。
这不代表完整斗地主、德州扑克、桥牌等已经验收通过；多人联网仍未实现。失败会明确展示，不能用同名比大小替代。
